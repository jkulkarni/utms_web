(function($) {
    ...
    $(document).on('add.cards change.cards', function(event) {
        if(typeof $.fn.masonry !== 'undefined') {
            $(event.target).outerFind('.ctrans-gallery').each(function() {
                var $msnr = $(this).find('.ctrans-gallery-row').masonry({
                    itemSelector: '.ctrans-gallery-item',
    ...
    function fitLightbox() {
        var $lightbox = $('.ctrans-gallery .modal');
        if(!$lightbox.length) {
            return;
        }
